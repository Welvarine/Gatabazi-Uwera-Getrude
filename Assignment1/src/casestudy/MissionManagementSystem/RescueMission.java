...
public class RescueMission extends Mission {
    ...
    public RescueMission() {
        System.out.println("Focuses on rescuing people from hazardous situations.");
        System.out.println("---------<<<>>>>>-------------------<<<>>>>--------------------<<<>>>---------------------");
        System.out.println("You must ENTER ATLEAST 1 MEDIC FOR THE MISSION");
        int minPeople = 1;
        int count = 1;
        boolean momo = true;
        while (momo) {
            System.out.println("Enter Medic " + count + " Details");
            while (true) {
        ...
        while (endDate.before(startDate)) {
            System.out.println("End date cannot be before start date.Re-enter.");
            endDate = getDateInput("End Date (yyyy-MM-dd): ");
            setEndDate(endDate);
        }
        boolean recon = true;
        while (recon) {
            System.out.println("RESCUE MISSION OPERATIONS");
            ...
            switch (choice3) {
                case 1:
                    ...
                    sc.nextLine();
                    System.out.println("Do you want to perform other Rescue Mission Operations?");
    ...
    public void assignTask(Personnel personnel) {
        ...
        System.out.println("---------------");
        System.out.println("TASK 1: RESCUE");
        System.out.println("TASK 2: MEDICAL");
        System.out.println("TASK 3: LOGISTICS");
        System.out.println("Select the Task Number");
        int task = sc.nextInt();
        if (task == 1) {
            System.out.println("Task is available");
            personnel.setPersonnelTask("RESCUE");
        } else if (task == 2) {
            personnel.setPersonnelTask("MEDICAL");
        } else if (task == 3) {
            personnel.setPersonnelTask("lOGISTICS");
        } else {
    ...
    public void trackMissionProgress() {
        ...
        System.out.println("---------------");
        System.out.println("Status 1: RESCUE COMPLETE");
        System.out.println("Status 2: INJURED TREATED");
        System.out.println("Enter the status number of the Mission");
        int stat = sc.nextInt();
        switch (stat) {
            case 1:
                setStatus("RESCUE COMPLETE");
                break;
            case 2:
                setStatus("INJURED TREATED");
                break;
            default:
